...
import { LightningSaleItemResponse } from "@/types/lightning-sale";
...
export default function LightningSaleCard({
  ...
  const primaryImage: string = getFileUrl(
    lightningSaleItem.plant.images.find(
      (img: PlantImageResponse) => img.isPrimary,
    )?.media?.fileUrl,
  );
      ...
      <div className={cn("group grid grid-cols-11 gap-4", className)}>
        <div className="relative col-span-5">
          ...
          >
            ...
            />
            <div className="w-fit bg-[#F7F70B] text-[#2A2A2A] text-xs font-medium px-2 py-1 absolute top-2 right-2 rounded-xs">{`GIẢM ${Math.round(lightningSaleItem.discountPercentage)}%`}</div>
        ...
        <div className="col-span-6 flex flex-col gap-2 justify-between">
          <div>
            ...
            <p className="text-2xl font-semibold text-primary mb-1">{`GIẢM ${Math.round(lightningSaleItem.discountPercentage)}%`}</p>
